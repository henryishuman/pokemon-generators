...
from models.model_utils.multilayered_perceptron import MLP


class MultilayeredPerceptronModel(Model):
    def __init__(self, window_size: int = 3, is_window_centred: bool = False, hidden_layer_nodes: List[int] = [3, 3]):       
    ...
    def train(self, df_train: pd.DataFrame, epochs=100, learning_rate=0.01) -> None:
        ...
        self.classifier = MLP(
            ...
            hidden_sizes=self.hidden_layer_nodes, 
            output_size=len(self.unique_values)
        )
        self.classifier_metrics = self.classifier.train(
            network_input[columns].to_numpy(), 
            network_input[['class']].to_numpy(),
            num_epochs=epochs,
    ...
    def generate(self, seed: List[int] = None, epochs: int = 1) -> List[int]:
        ...
        for _ in range(epochs):
            for pix_ix in range(self.image_size):
                ...
                output = self.classifier.predict(df_predict[columns].to_numpy())
                activated_neuron = max(range(len(output)), key=output.__getitem__)
                generated_image[pix_ix] = int(self.unique_values[activated_neuron])
